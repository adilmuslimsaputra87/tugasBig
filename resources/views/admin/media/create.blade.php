...
<body>
    <div class="admin-page">
        <div class="admin-header">
            ...
            <h1 class="admin-title">TAMBAH MEDIA</h1>
        </div>

        <form class="form-container" method="POST" action="/simpanMedia" enctype="multipart/form-data">
            @csrf

            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Nama Media</label>
                    <input type="text" name="name" class="form-input" placeholder="Nama Media" value="{{ old('name') }}" required>
                    @error('name') <span style="color: var(--red); font-size: 12px;">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Lokasi</label>
                    <input type="text" name="location" class="form-input" placeholder="Lokasi Media" value="{{ old('location') }}" required>
